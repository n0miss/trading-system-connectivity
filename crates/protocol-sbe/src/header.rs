...
mod tests {
    ...
    fn make_header_bytes(block_length: u16, template_id: u16, schema_id: u16, version: u16) -> [u8; 8] {
        let mut buf = [0u8; 8];
    ...
    fn validate_schema_correct_schema_and_version_ok() {
        let hdr = SbeHeader { block_length: 58, template_id: 0,
                              schema_id: SPOT_SCHEMA_ID, version: 0 };
        assert!(hdr.validate_schema().is_ok());
    }

    #[test]
    fn validate_schema_max_supported_version_ok() {
        let hdr = SbeHeader { block_length: 0, template_id: 0,
                              schema_id: SPOT_SCHEMA_ID, version: SPOT_SCHEMA_VERSION_MAX };
        assert!(hdr.validate_schema().is_ok());
    }

    #[test]
    fn validate_schema_wrong_schema_id_errors() {
        let hdr = SbeHeader { block_length: 0, template_id: 0, schema_id: 99, version: 0 };
        let err = hdr.validate_schema().unwrap_err();
        assert_eq!(err, SbeError::SchemaMismatch { expected: SPOT_SCHEMA_ID, actual: 99 });
    }
    ...
    fn validate_schema_version_too_new_errors() {
        ...
        let err = hdr.validate_schema().unwrap_err();
        assert_eq!(err, SbeError::VersionTooNew {
            max: SPOT_SCHEMA_VERSION_MAX, actual: SPOT_SCHEMA_VERSION_MAX + 1,
        });
    }

    #[test]
    fn validate_schema_schema_id_zero_errors() {
        let hdr = SbeHeader { block_length: 0, template_id: 0, schema_id: 0, version: 0 };
        assert!(matches!(hdr.validate_schema(), Err(SbeError::SchemaMismatch { .. })));
    }
}
