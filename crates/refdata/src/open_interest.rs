...
use crate::{client::RestClient, error::RefDataError, normalizer::normalize_open_interest};
...
impl OpenInterestPoller {
    pub fn new(client: RestClient, symbols: Vec<InstrumentDefinition>, interval: Duration) -> Self {
        Self {
            client,
            symbols,
            interval,
    ...
    async fn poll_all(&mut self, on_msg: &mut impl FnMut(OpenInterest)) {
        ...
        for i in 0..self.symbols.len() {
            ...
            match result {
                Ok(resp) => match normalize_open_interest(&resp, &self.symbols[i], self.next_seq) {
                    Ok(msg) => {
                        self.next_seq += 1;
                        on_msg(msg);
                    }
