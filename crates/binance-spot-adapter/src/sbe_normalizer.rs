use connector_core::{
    AggressorSide as CoreAggressorSide,
    BestBidOffer, BookDelta, InstrumentDefinition, MessageHeader,
    MessageType, NormalizedMessage, PriceLevel, Trade,
    SCHEMA_VERSION, TS_NONE, UPDATE_ID_NONE,
};
use protocol_sbe::{
    AggressorSide as SbeAggressorSide,
    BboEvent, DepthDiffEvent, SbeMessage, TradeEvent,
};
...
pub fn normalize_sbe_message(
    ...
    match msg {
        SbeMessage::Trade(ev)        => {
            Some(normalize_trade(ev, inst, ctx, take_seq(seq), recv_ts))
        }
        SbeMessage::Bbo(ev)          => {
            Some(normalize_bbo(ev, inst, ctx, take_seq(seq), recv_ts))
        }
...
fn normalize_depth_diff(
    ...
    let bids = ev.bids.into_iter()
        .map(|lvl| PriceLevel { price: lvl.price.to_scaled(p), qty: lvl.quantity.to_scaled(q) })
        .collect();

    let asks = ev.asks.into_iter()
        .map(|lvl| PriceLevel { price: lvl.price.to_scaled(p), qty: lvl.quantity.to_scaled(q) })
        .collect();
...
mod tests {
    ...
    use protocol_sbe::{
        AggressorSide as SbeAggressorSide, BboEvent, Decimal64, DepthDiffEvent,
        DepthLevel, DepthSnapshotEvent, SbeMessage, TradeEvent,
    };
    ...
    fn d64(mantissa: i64) -> Decimal64 { Decimal64 { mantissa } }
    ...
    fn trade_header_fields() {
        ...
        let msg = normalize_sbe_message(
            SbeMessage::Trade(btcusdt_trade_event()), &inst, &ctx, &mut seq, 999,
        ).unwrap();
    ...
    fn trade_price_scaled_at_2() {
        ...
        let mut seq = 0u64;
        let msg = normalize_sbe_message(
            SbeMessage::Trade(btcusdt_trade_event()), &inst, &ctx, &mut seq, 0,
        ).unwrap();
    ...
    fn bbo_header_and_timestamps() {
        ...
        let mut seq = 5u64;
        let msg = normalize_sbe_message(
            SbeMessage::Bbo(btcusdt_bbo_event()), &inst, &ctx, &mut seq, 42,
        ).unwrap();
        let NormalizedMessage::BestBidOffer(bbo) = msg else { panic!() };
    ...
    fn bbo_prices_scaled_at_2() {
        ...
        let mut seq = 0u64;
        let msg = normalize_sbe_message(
            SbeMessage::Bbo(btcusdt_bbo_event()), &inst, &ctx, &mut seq, 0,
        ).unwrap();
        let NormalizedMessage::BestBidOffer(bbo) = msg else { panic!() };
    ...
    fn btcusdt_diff_event() -> DepthDiffEvent {
        DepthDiffEvent {
            ...
            symbol:               "BTCUSDT".to_string(),
            bids: vec![
                DepthLevel { price: d64(9_650_000_000_000), quantity: d64(2_500_000_000) },
                DepthLevel { price: d64(9_649_900_000_000), quantity: d64(0) },
            ],
            asks: vec![
                DepthLevel { price: d64(9_650_100_000_000), quantity: d64(1_000_000_000) },
            ],
        }
    }
    ...
    fn depth_diff_header_and_update_ids() {
        ...
        let mut seq = 10u64;
        let msg = normalize_sbe_message(
            SbeMessage::DepthDiff(btcusdt_diff_event()), &inst, &ctx, &mut seq, 1,
        ).unwrap();
    ...
    fn depth_diff_bid_levels_scaled() {
        ...
        let mut seq = 0u64;
        let msg = normalize_sbe_message(
            SbeMessage::DepthDiff(btcusdt_diff_event()), &inst, &ctx, &mut seq, 0,
        ).unwrap();
        let NormalizedMessage::BookDelta(bd) = msg else { panic!() };
