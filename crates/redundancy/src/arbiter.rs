...
impl ChecksumArbiter {
    ...
    pub fn pending_count(&self, symbol: &str) -> usize {
        self.symbols
            ...
            .map(|q| {
                q.iter()
                    .filter(|e| e.active.is_none() || e.passive.is_none())
                    .count()
            })
            .unwrap_or(0)
    }
...
pub fn process<T: FailoverTrigger>(
    ...
    if let ArbiterVerdict::Diverged {
        ref symbol,
        update_id,
        active_checksum,
        passive_checksum,
...
mod tests {
    ...
    fn active(symbol: &str, update_id: u64, checksum: u64) -> BookChecksum {
        msg(0, symbol, update_id, checksum)
    }
    fn passive(symbol: &str, update_id: u64, checksum: u64) -> BookChecksum {
        msg(1, symbol, update_id, checksum)
    }
    ...
    impl FailoverTrigger for RecordingTrigger {
        fn on_divergence(&self, symbol: &str, update_id: u64, a: u64, p: u64) {
            self.calls
                .borrow_mut()
                .push((symbol.into(), update_id, a, p));
        }
    }
    ...
    fn verdict_predicates() {
        let matched = ArbiterVerdict::Matched {
            symbol: "S".into(),
            update_id: 1,
            checksum: 0,
        };
