...
impl CircuitBreaker {
    pub fn new() -> Self { Self::with_limits(MAX_ATTEMPTS, COOLDOWN_NS) }

    pub fn with_limits(max_attempts: u32, cooldown_ns: i64) -> Self {
        Self { failures: 0, max_attempts, cooldown_ns, open_until: None }
    }
    ...
    pub fn record_failure(&mut self, now_ns: i64) -> bool {
        if self.open_until.is_some() { return false; }
        self.failures += 1;
...
mod tests {
    ...
    fn failures_below_max_do_not_open() {
        ...
        for _ in 0..4 { assert!(!cb.record_failure(0)); }
        assert_eq!(cb.check(0), CircuitState::Closed);
    }
    ...
    fn max_failures_opens_circuit() {
        ...
        for _ in 0..2 { cb.record_failure(0); }
