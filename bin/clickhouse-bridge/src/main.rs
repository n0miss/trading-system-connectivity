    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            unsafe {
                ...
                let mut sig = 0i32;
                libc::sigwait(
                    &wait_mask as *const libc::sigset_t,
...
#[command(
    ...
    about = "Aeron → ClickHouse market data bridge"
)]
struct Args {
...
impl Inserters {
    fn new(client: &Client, batch_rows: u64, batch_period: Duration) -> Self {
        Self {
            ...
            bbo: client
                .inserter("best_bid_offers")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
            mark_price: client
                .inserter("mark_prices")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
            funding: client
                .inserter("funding_rates")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
            liquidation: client
                .inserter("liquidations")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
            open_interest: client
                .inserter("open_interest")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
            book_delta: client
                .inserter("book_deltas")
                .with_max_rows(batch_rows)
                .with_period(Some(batch_period)),
    ...
    async fn write(&mut self, msg: &NormalizedMessage) {
        let result: clickhouse::error::Result<()> = match msg {
            ...
            NormalizedMessage::Liquidation(m) => {
                self.liquidation.write(&LiquidationRow::from(m)).await
            }
            NormalizedMessage::OpenInterest(m) => {
                self.open_interest.write(&OpenInterestRow::from(m)).await
            }
